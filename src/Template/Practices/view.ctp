...
<div class="practices view large-12 medium-11 columns content">
    <h3><i class="fas fa-chalkboard fa-1x fa-fw"></i>&nbsp;&nbsp;<?= h($practice->title) ?><div class="btn-group btn-group-sm pull-right" role="group" aria-label="...">
      <?= $this->Html->link(__('Practices'), ['controller' => 'practices', 'action' => 'index','plugin'=>false], ['class'=>'btn btn-default ']);?>
      <?= $this->Html->link(__('Month'), ['controller' => 'months', 'action' => 'view',$practice->month_id, 'plugin'=>false], ['class'=>'btn btn-default ']);?>
    </div></h3>
