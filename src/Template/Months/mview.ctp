...
<h3><i class="fas fa-calendar-alt fa-1 fa-fw"></i>&nbsp;&nbsp;<?= $month->title." ".$month->year; ?>
...
<div class="panel panel-primary">
        ...
        <table  class="table table-striped" cellpadding="0" cellspacing="0">
            <thead>
                ...
                <th style="text-align: center" scope="col"><?= __('Visible') ?></th>
                <th style="text-align: center" scope="col"><?= __('Sign Ups Open') ?></th>
            </thead>
            <?php foreach ($month->shows as $shows): ?>
            <tr>
                <td><?= $this->Html->link(__(''), ['controller' => 'Shows', 'action' => 'mview', $shows->id], ['class'=>'fas fa-plus fa-l  fa-fw text-success', 'title'=>'Make Assignment']) ?></td>
                <td><?= $this->Html->link(__(''), ['controller' => 'Shows', 'action' => 'view', $shows->id], ['class'=>'fas fa-eye fa-l  fa-fw text-success', 'title'=>'View Show']) ?></td>
                <td><?= $this->Html->link(__(''), ['controller' => 'Shows', 'action' => 'edit', $shows->id], ['class'=>'fas fa-edit fa-l  fa-fw text-success', 'title'=>'Edit Show']) ?></td>
                <td><?= $this->Form->postLink(__(''), ['controller' => 'Shows', 'action' => 'remove', $shows->id], ['confirm' => __('Are you sure you want to delete this show?', $shows->id), 'class'=>'fas fa-trash-alt fa-l  fa-fw text-danger', 'title'=>'Delete Show']) ?></td>
                <td><?= h($shows->Dropdowns['name']) ?></td>
                <td><?= h($this->Switches->datetime($shows->schedule)) ?></td>
                <td style="text-align: center"><?= h($shows->numberOfSignups()) ?></td>
                <td style="text-align: center"><?= $shows->visible ? "<i class='fas fa-check-circle fa-l fa-fw text-success' text-success'></i>" : '' ?></td>
                <td style="text-align: center"><?= $shows->signups_open ? "<i class='fas fa-check-circle fa-l fa-fw text-success' text-success'></i>" : '' ?></td>
            </tr>
            <?php endforeach; ?>
            <tfoot>
                <td colspan="9">&nbsp;</td>
