...
<div class="panel panel-primary">
                <div class="panel-heading">
                    ...
                    <span class="pull-right">
                        ...
                        <ul class="nav panel-tabs">
                            <li class="active"><a href="#tab1" data-toggle="tab"><i class="fas fa-calendar fa-l fa-fw"></i>&nbsp;&nbsp;Shows</a></li>
                            <li><a href="#tab2" data-toggle="tab"><i class="fas fa-pen-alt fa-l fa-fw"></i>&nbsp;&nbsp;Sign Ups</a></li>
                            <li><a href="#tab3" data-toggle="tab"><i class="fas fa-chalkboard-teacher fa-l fa-fw"></i>&nbsp;&nbsp;Practices</a></li>
                        </ul>
                    </span>
                </div>
        ...
        <table  class="table table-striped" cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td>
                <?php if($shows->visible == 1) : ?> <?= $this->Html->link(__(''), ['controller'=>'Shows', 'action' => 'view', $shows->id], ['class'=>'fas fa-eye fa-l fa-fw text-success', 'title'=>'View Show'])?>
                    <?php endif; ?>
                </td>
                <td>
                <?php if($shows->visible == 1) : ?> <?= $this->Html->link(__(''), ['controller'=>'Shows', 'action' => 'signup', $shows->id], ['class'=>'fas fa-pen-alt fa-l fa-fw text-success', 'title'=>'View and Sign Up'])?>
                <?php endif; ?>
                </td>
                <td>
                <?php if($shows->signups_open == 1 && $shows->visible == 1) : ?> <?= $this->Form->create(null, [
                ...
                <td><?php if (!empty($shows->show_url)) : ?> <a href='<?= $shows->show_url ?>' class ="fab fa-chrome fa-l fa-fw text-success" target="_blank" title='Show URL'></a> <?php endif;?>
                </td>
                <td><?= h($shows->Dropdowns['name']) ?></td>
                <td><?= h($shows->DisplayName) ?></td>
                <td style="text-align: center;"><?= $shows->numberOfSignups() ?>
                ...
                <?= $shows->players_needed ?> <?php if($shows->numberOfSignups() > $shows->players_needed || $shows->numberOfSignups() == $shows->players_needed) echo "<i class='fas fa-check fa-l fa-fw text-success'></i>";  ?>
            <?php endif ;?>
                </td>
                <td style="text-align: center;"><?= $shows->ref_needed ? '   '.$this->Html->image('ref.png', ['title'=>'need a ref']).'  ' : $this->Html->image('need.png').'  ' ?><?= $shows->voice_needed ? '   '.$this->Html->image('voice.png', ['title'=>'need a voice']).'  ' : '   '.$this->Html->image('need.png').'  '?><?= $shows->host_needed ? '   '.$this->Html->image('host.png', ['title'=>'need a host']) : '   '.$this->Html->image('need.png') ?></td>
            </tr>
            <?php endforeach; ?>
            <tfoot>
                <td colspan="8">    <div class="paginator" style="display:none;">
        <ul class="pagination">
...
<?php if (!empty($signups)): ?>
        <table  class="table table-striped" cellpadding="0" cellspacing="0">
            <thead>
                ...
                <th scope="col"><?= __('Signed Up On')?></th>
            </thead>
            ...
            <tr>
                ...
                <td><?= h($this->Switches->datetime($signups->created)) ?></td>
            </tr>
            <?php endforeach; ?>
            <tfoot>
                <td colspan="4">    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
        ...
        <table  class="table table-striped" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <td><?= h($this->Switches->datetime($practices->schedule)) ?></td>
